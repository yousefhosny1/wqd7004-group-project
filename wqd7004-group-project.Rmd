...
### 6.2.2 Diamond Cut Classification using Decision Trees
```{r}
library(caTools)
...
plot(classifier_RF)
```
As the number of trees increase, it can be seen that the error rate destablised.
```{r}
importance(classifier_RF)
```
We can see that the attribute table is the most significant attribute which is then followed by depth. 
```{r}
varImpPlot(classifier_RF)
```
The plot shows that the attribute table is the most significant feature which is then followed by depth. 
